...
# BCNS protocol CDF lookup table
rlwe_table = [
...
for row in rlwe_table:
    big = 0
    for (element, j) in zip(row, range(len(row))):
        big = big | (element << (j * 64))
    bigIntegers.append(big)
   
bar_chart(bigIntegers).show()   # create a  bar chart of CDF array

def sample():   # this function uses uniformly random bits to sample Gaussian 
    n = random.getrandbits(192)
    for index in range(len(bigIntegers)):
        if n < bigIntegers[index]:
            return index
            
array = [0] * len(bigIntegers)  # create empty array with size = length of array
for i in range(100000):         # count of samples
    s = sample() 
    array[s] += 1
    
bar_chart(array).show()         # create bar chart of samples (using occurrences)